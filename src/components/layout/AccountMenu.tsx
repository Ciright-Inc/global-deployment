...
import {
  keyraConsumerAppHref,
  keyraConsumerAppIsExternal,
...
function ConsumerAppLink({
  ...
  const base = keyraConsumerAppHref().replace(/\/+$/, "");
  const href = path ? `${base}${path.startsWith("/") ? path : `/${path}`}` : base;
  const external = keyraConsumerAppIsExternal();
  if (external) {
    return (
      <a
        role="menuitem"
    ...
    <div className="relative shrink-0" ref={wrapRef}>
      ...
      {open ? (
        ...
        >
          ...
          <ConsumerAppLink path="/family" onNavigate={() => setOpen(false)}>
            Family
          </ConsumerAppLink>
          <ConsumerAppLink path="" onNavigate={() => setOpen(false)}>
            My Keyra
          </ConsumerAppLink>
          <button
            type="button"

...
export function KeyraAppLauncher() {
  ...
  return (
    <div className="relative z-[calc(var(--keyra-z-header)+1)] shrink-0" ref={wrapRef}>
      <button
        type="button"
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      {open ? (
        <div
          role="menu"
          aria-label="Keyra apps"
          className="absolute right-0 top-[calc(100%+8px)] z-[calc(var(--keyra-z-header)+2)] flex max-h-[min(34rem,calc(100dvh-6rem))] min-h-0 w-[min(calc(100vw-1.5rem),20rem)] flex-col overflow-hidden rounded-xl border border-black/12 bg-keyra-bg p-3 shadow-[0_24px_64px_rgba(0,0,0,0.14),0_0_0_1px_rgba(0,0,0,0.05)] sm:w-[20rem]"
        >
